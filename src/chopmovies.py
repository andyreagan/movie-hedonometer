...
def precomputeTimeseries(fullVec, labMT, labMTvector, outfile):
    ...
    for i in xrange(len(timeseries) - 1, len(timeseries) + minWindows / 2):
        ...
        # print("removing point {0}".format(i-minWindows))
        textFvec = [
            textFvec[j] - fullVec[j][i - minWindows] for j in xrange(len(fullVec))
        ]
...
def renameThe(folder):
    ...
    for movie in query:
        ...
        if not correctname == wrongname:
            try:
                shutil.move(
                    "data/scripts/" + folder + "/" + wrongname + ".html",
                    "data/scripts/" + folder + "/" + correctname + ".html",
                )
            except:
...
def renameFull(folder):
    ...
    for line in f:
        ...
        print(messytitle)
        shutil.move(
            "data/scripts/" + folder + "/" + messytitle,
            "data/scripts/" + folder + "/" + cleantitle,
        )
    f.close()
    g.close()
...
def checkClean(folder):
    ...
    for line in f:
        ...
        fsize = os.stat(
            "data/scripts/" + folder + "/" + cleantitle + ".end.beg"
        ).st_size
        if fsize < 4000:
            print(messytitle)
...
def process():
    ...
    for movie in query:
        ...
        if os.path.isfile(
            ...
            print(movie.title)
            print(
                "opening scriptsClean/"
                + titleraw.replace(" ", "-").replace(".", "-")
                + ".txt"
            )
            f = codecs.open(
                "scriptsClean/" + titleraw.replace(" ", "-").replace(".", "-") + ".txt",
                "r",
                "utf8",
            )
            ...
            for window in windowSizes:
                ...
                breaks = [
                    klines[window / 10 * i]
                    for i in xrange(int(floor(float(len(klines)) / window * 10)))
                ]
                breaks[0] = 0
                ...
                f = open(
                    "word-vectors/" + str(window) + "/" + movie.filename + ".csv", "r"
                )
...
def process_overallHapps():
    ...
    for movie in query:
        ...
        stoppedVec = stopper(
            textFvec, labMTvector, labMTwordList, stopVal=0.0, ignore=ignoreWords
        )
    ...
    f = open("data/word-vectors/full/" + m.filename + ".csv", "w")
...
def testRE():
    ...
    for movie in query:
        ...
        if os.path.isfile("data/scripts/scriptsClean/" + filename + ".txt"):
            print(movie.title)
            f = codecs.open(
                "data/scripts/scriptsClean/" + filename + ".txt", "r", "utf8"
            )
            raw_text = f.read()
            f.close()
...
def fixModels():
    f = open("data/metadata/titles-clean.txt", "r")
    titles = [line.rstrip() for line in f]
    f.close()
    f = open("data/metadata/titles-raw.txt", "r")
